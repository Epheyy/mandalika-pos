...
import type { AppUser, Customer, Order, LoyaltySettings } from '../../../types'
...
import { Plus, Search, Edit2, Trash2, X, Users, Phone, Mail, Star, Loader2, ChevronDown, ChevronUp, ShoppingBag, Settings2, ToggleLeft, ToggleRight } from 'lucide-react'
...
export default function Customers({ appUser: _appUser }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubLoyalty = onSnapshot(doc(db, 'settings', 'loyalty'), snap => {
      if (snap.exists()) {
        setLoyalty(snap.data() as LoyaltySettings)
        setLoyaltyForm(snap.data() as LoyaltySettings)
      }
    })
  ...
  const saveLoyaltySettings = async () => {
    ...
    try {
      await setDoc(doc(db, 'settings', 'loyalty'), loyaltyForm)
      alert('Pengaturan loyalitas disimpan!')
      ...
      {activeTab === 'settings' ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5 max-w-lg">
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
            ...
            <button onClick={() => setLoyaltyForm(f => ({ ...f, enabled: !f.enabled }))}
          ...
          <div className="flex items-center justify-between py-3 border-b border-gray-100">
            ...
            <button onClick={() => setLoyaltyForm(f => ({ ...f, tierEnabled: !f.tierEnabled }))}
              className="text-gray-400 hover:text-indigo-600 transition-colors">
            ...
            {isLoading ? <div className="p-8 text-center text-gray-300">Memuat...</div>
              : filtered.length === 0 ? (
                <div className="p-12 flex flex-col items-center gap-3 text-gray-300">
                  ...
                  <p className="font-bold">Tidak ada pelanggan</p>
                </div>
              ) : (
                        ...
                        <div className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 cursor-pointer transition-all"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400">
                              {format(parseISO(order.createdAt), 'dd MMM yyyy HH:mm', { locale: idLocale })} · <span className="capitalize">{order.paymentMethod}</span>
                            </p>
                          </div>
